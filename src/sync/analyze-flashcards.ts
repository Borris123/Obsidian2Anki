	...
	for (const flashcard of flashcards) {
		...
		if (noteId === undefined) {
			...
			if (canAdd) {
				...
				operations.push({
					type: "skip", flashcard, reason: "duplicate",
				});
			}
			continue;
		}
		...
		if (!ankiNote) {
			...
			if (canAdd) {
				operations.push({
					type: "create", flashcard, previousNoteId: noteId,
				});
			} else {
				operations.push({
					type: "skip", flashcard, reason: "duplicate", previousNoteId: noteId,
				});
			}
			continue;
		}
