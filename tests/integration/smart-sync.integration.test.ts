...
import {parseFlashcards,} from "../../src/flashcards/flashcard-parser";

import type {AnkiNote,} from "../../src/anki/anki-note";

import type {FlashcardSyncClient,} from "../../src/sync/sync-flashcards";
import {syncFlashcards,} from "../../src/sync/sync-flashcards";
...
describe("smart sync integration", () => {
	...
	it("creates new flashcards and writes their Anki IDs to Markdown", async () => {
		...
		const markdown = `Array :: Collection of elements
...
Stack :: LIFO`;
		...
		expect(result.updatedMarkdown,).toBe(`<!-- anki-note-id:100 -->
...
Stack :: LIFO`,);
	...
	it("skips unchanged existing flashcards", async () => {
		...
		const markdown = `<!-- anki-note-id:100 -->
Array :: Collection of elements`;
		...
		client.getNotes
			.mockResolvedValue([createAnkiNote(100, "Array", "Collection of elements",),]);
...
Array :: Contiguous collection`;
		...
		expect(client.updateFlashcard,).toHaveBeenCalledWith(100, {
			front: "Array",
...
Queue :: FIFO`;
		...
		client.getNotes
			.mockResolvedValue([createAnkiNote(100, "Array", "Old collection",),

				createAnkiNote(200, "Stack", "LIFO",),]);
		...
		expect(client.updateFlashcard,).toHaveBeenCalledWith(100, expect.objectContaining({
			front: "Array",

			back: "Updated collection",
		}),);

		expect(result.updatedMarkdown,).toContain("<!-- anki-note-id:300 -->\n" + "Queue :: FIFO",);
	});
});
